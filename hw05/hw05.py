...
class Coin:
    ...
    def worth(self):
        extra_cent = max(0,Mint.current_year - self.year - 50)
...
def is_bst(t):
    ...
    def bst_min(t):
        if t.is_leaf():
            return t.label
        entries = list(map(bst_min,t.branches))
        return min([t.label] + entries)
