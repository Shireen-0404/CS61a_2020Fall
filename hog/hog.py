...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    def say_running_highest(score0,score1):
        if who == 0:
            score = score0
        else:
            score = score1
        if score - last_score > running_high:
            print(score - last_score,'poing(s)! The most yet for Player',who)
            return announce_highest(who,score,score - last_score)
        else:
            return announce_highest(who,score,running_high)
    return say_running_highest
    # END PROBLEM 7

def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
         goal=GOAL_SCORE, say=both(announce_highest(0),both(say_scores, announce_lead_changes()))):
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    while rolls > 0:
        if max_average <= average(rolls,dice):
            max_average = average(rolls,dice)
            num_rolls_for_max = rolls
        rolls = rolls - 1
    return num_rolls_for_max
    # END PROBLEM 9
