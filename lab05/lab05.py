...
def berry_finder(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    if 'berry' in str(t):#str converts all the elements in the tree into a node, even the [] and , are vlues of nodes
                return True
    else:
        return False
...
def sprout_leaves(t, leaves):
    ...
    """
    "*** YOUR CODE HERE ***"
    branch_leaf = [tree(i) for i in leaves]
    if is_leaf(t):
        t = tree(label(t),branch_leaf)
        return t
...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [deck[(len(deck)//2+1)**(i%2)-1+i//2] for i in range(len(deck))]


def add_trees(t1, t2):#不会
    """
    >>> numbers = tree(1,
    ...                [tree(2,
    ...                      [tree(3),
...
def build_successors_table(tokens):
    ...
    for word in tokens:
        if prev not in table:
            table[prev] = [word]#change word into a list
        else:
...
def construct_sent(word, table):
    ...
    result = ''
    while word not in ['.', '!', '?']:
        result = result + word + ' ' #add a space between words
        word = random.choice(table[word])
    return result.strip() + word #strip the space
