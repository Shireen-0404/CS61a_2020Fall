...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(index,dir,k):
        if index==n:
            return k
        elif index%8==0 or num_eights(index)>0:
            return helper(index+1,-dir,k-dir)
        else:
            return helper(index+1,dir,k+dir)
    return helper(1,1,1)#这里完成了assignment并给出base case:index=1时，pingpongvalue=1，direction是countup
