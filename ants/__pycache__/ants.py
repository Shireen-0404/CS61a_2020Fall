...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            #assert isinstance(self,ContainerAnt) != isinstance(place.ant,ContainerAnt), 'Two ants in {0}'.format(place)
            if self.can_contain(place.ant):
                self.contain_ant(place.ant)
                place.ant = self#提示unreacherable，但运行没问题，设置place的ant参数
            elif place.ant.can_contain(self):
                place.ant.contain_ant(self)
            else:
                assert False, 'Two ants in {0}'.format(place)
            # END Problem Optional 2
        Insect.add_to(self, place) #设置insect的place参数，可以有2只蚂蚁的place参数相同，但place的ant参数只有一个值
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        distance = 0
        check_entrance = self.place
        while check_entrance != beehive:
            if distance > self.max_range :
                return None
            elif distance >=self.min_range:
                if check_entrance.bees != []:
                    return rANTdom_else_none(check_entrance.bees)
            check_entrance = check_entrance.entrance
            distance +=  1
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        its armor to 0."""
        # BEGIN Problem 8
        Place.add_insect(self,insect)#failed by calling insect.add_to()
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        # BEGIN Problem EC
        self.is_true_queen = QueenAnt.is_true_queen#卡在了这里，用class name修改class attributes
    ...
    def action(self, gamestate):
        ...
        if self.is_true_queen:
            ...
            check_exit = self.place.exit
            while check_exit != None:
                if check_exit.ant != None and not hasattr(check_exit.ant,'mark'):
                    check_exit.ant.mark = True
                    if check_exit.ant.damage != None:
                        check_exit.ant.damage = 2*check_exit.ant.damage
                check_exit = check_exit.exit
        else:
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem Optional 2
        if (self.contained_ant == None) and not isinstance(other,ContainerAnt):#卡在这儿了，‘other is not ContainerAnt'
            return True
        else:
            return False
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        while check_place != beehive:
            if check_place.ant and check_place.ant is not self:
                all_insects[check_place.ant] = distance
            if check_place.bees:
                copy_bees = check_place.bees[:]
                for b in copy_bees:
